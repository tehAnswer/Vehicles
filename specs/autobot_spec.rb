...
describe Autobot do
  ...
  it 'is strapped' do
    expect(optimus.weapons?).to be_truthy

    expect(optimus.remaining_ammo).to eq(3)
    expect(optimus.shoot!).to be_truthy
    ...
    expect(optimus.shoot!).to be_falsey
  ...
  it 'is electric' do
    expect(optimus.electric?).to be_truthy
  end
  ...
  it 'has a name' do
    expect(optimus.name).to eq('Optimus Prime')
  end
